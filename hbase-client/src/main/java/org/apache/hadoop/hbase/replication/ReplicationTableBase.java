...
abstract class ReplicationTableBase {
  ...
  /*
  * Because many Replication Table operations abort the server on failure, we have to make sure that
  * ReplicationTable operations can handle longer periods of Replication Table region unavailability
  * Because RPC requests fail almost immediately on region unavailability, the amount of time we can
  * tolerate region unavailability is largely determined by (retries * pause). With the default
  * setup we can tolerate at least 10 minutes of region unavailability before aborting.
  */
  private static final int DEFAULT_CLIENT_RETRIES = 100;
  private static final int DEFAULT_CLIENT_PAUSE = 6000;
  private static final int DEFAULT_RPC_TIMEOUT = 60000;
  private static final int DEFAULT_SCANNER_TIMEOUT = DEFAULT_CLIENT_RETRIES * DEFAULT_CLIENT_PAUSE;
  ...
  */
  ...
  /**
   * Read in the configuration values that set up the Replication Table operation timeouts and
   * retries
   */
  private void readTimeoutConf() {
    ...
    operationTimeout = calculateTimeout(clientRetries, clientPause, rpcTimeout);
    initOperationTimeout = calculateTimeout(initRetries, initPause, initRpcTimeout);
  }

  private int calculateTimeout(int retries, int pause, int rpcTimeout) {
  ...
  private class CreateReplicationTableWorker implements Runnable {
    ...
    private Configuration buildTableInitConf() {
      ...
      tempConf.setInt(HConstants.HBASE_CLIENT_RETRIES_NUMBER, initRetries);
      tempConf.setInt(HConstants.HBASE_CLIENT_PAUSE, initPause);
      tempConf.setInt(HConstants.HBASE_RPC_TIMEOUT_KEY, initRpcTimeout);
      tempConf.setInt(HConstants.HBASE_CLIENT_OPERATION_TIMEOUT, initOperationTimeout);
      return tempConf;
    }
    ...
    private boolean replicationTableAvailable() {
      try {
        ...
        return initAdmin.tableExists(REPLICATION_TABLE_NAME) &&
            initAdmin.isTableAvailable(REPLICATION_TABLE_NAME);
      } catch (Exception e) {
        // Catches a null pointer exception that is thrown when admin.tableExists() is called too
        // early. In this case assume the Table is not available yet.
        return false;
      }
    }
  }
