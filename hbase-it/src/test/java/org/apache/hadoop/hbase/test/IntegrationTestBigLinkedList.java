...
public class IntegrationTestBigLinkedList extends IntegrationTestBase {
  ...
  private static final String CONCURRENT_WALKER_PRIORITY_KEY
    = "IntegrationTestBigLinkedList.generator.concurrentwalker.priority";
  private static final String CONCURRENT_WALKER_RANDOM_KEY
    = "IntegrationTestBigLinkedList.generator.concurrentwalker.random";
  ...
  private static final int CONCURRENT_WALKER_PRIORITY_DEFAULT = 0;
  private static final int CONCURRENT_WALKER_RANDOM_DEFAULT = 0;
  ...
  static class Generator extends Configured implements Tool {
    ...
    static class GeneratorMapper
      ...
      int numPriorityWalkers;
      int numRandomWalkers;
      volatile Stack<Long> priorityFlushedLoops = new Stack<>();
      volatile List<Long> allFlushedLoops = new ArrayList<>();
      ...
      protected void setup(Context context) throws IOException, InterruptedException {
        ...
        this.numPriorityWalkers = context.getConfiguration().getInt(CONCURRENT_WALKER_PRIORITY_KEY,
            CONCURRENT_WALKER_PRIORITY_DEFAULT);
        this.numRandomWalkers = context.getConfiguration().getInt(CONCURRENT_WALKER_RANDOM_KEY, CONCURRENT_WALKER_RANDOM_DEFAULT);
      ...
      protected void map(BytesWritable key, NullWritable value, Context output) throws IOException {
        ...
        if (++i == current.length) {
          ...
          if (count % wrap == 0) {
            ...
            // concurrent walker
            if (numPriorityWalkers > 0 || numRandomWalkers > 0) {
              addFlushed(key.getBytes());
              if (walkers.isEmpty()) {
                startWalkers(numPriorityWalkers, numRandomWalkers, conf, output);
              }
            }
            first = null;
            prev = null;
      ...
      private void addFlushed(byte[] rowKey) {
        synchronized (priorityFlushedLoops) {
          priorityFlushedLoops.push(Bytes.toLong(rowKey));
          priorityFlushedLoops.notifyAll()
          ;
        }
        synchronized (allFlushedLoops) {
          allFlushedLoops.add(Bytes.toLong(rowKey));
          allFlushedLoops.notifyAll();
        }
      }
      ...
      private void startWalkers(int numPriorityWalkers, int numRandomWalkers, Configuration conf,
          Context context) {
        LOG.info("Starting " + numPriorityWalkers + " priority walkers");
        LOG.info("Starting " + numRandomWalkers + " random walkers");
        for (int i = 0; i < numPriorityWalkers; i++) {
          Thread walker = new Thread(new ContinuousPriorityWalker(conf, context));
          walker.start();
          walkers.add(walker);
        }
        for (int i = 0; i < numRandomWalkers; i++) {
          Thread walker = new Thread(new ContinuousRandomWalker(conf, context));
          walker.start();
          walkers.add(walker);
        }
      }
      ...
      abstract class ContinuousConcurrentWalker implements Runnable {
        ...
        protected abstract long selectLoop () throws InterruptedException;
      }

      public class ContinuousRandomWalker extends ContinuousConcurrentWalker{
        Random rand;
        public ContinuousRandomWalker(Configuration conf, Context context) {
          super(conf, context);
          rand = new Random();
        }
        @Override
        protected long selectLoop () throws InterruptedException{
          synchronized (allFlushedLoops) {
            while (allFlushedLoops.isEmpty() && !walkersStop) {
              allFlushedLoops.wait();
            }
            if (walkersStop) {
              throw new InterruptedException();
            }
            return allFlushedLoops.get(rand.nextInt(allFlushedLoops.size()));
          }
        }
      }

      public class ContinuousPriorityWalker extends ContinuousConcurrentWalker{
        public ContinuousPriorityWalker(Configuration conf, Context context) {
          super(conf, context);
        }
        @Override
        protected long selectLoop () throws InterruptedException{
          synchronized (priorityFlushedLoops) {
            while (priorityFlushedLoops.empty() && !walkersStop) {
              priorityFlushedLoops.wait();
            }
            if (walkersStop) {
              throw new InterruptedException();
            }
            return priorityFlushedLoops.pop();
          }
        }
      }
    ...
    public int run(String[] args) throws Exception {
      ...
      try {
        ...
        Integer numPriorityWalkers = (args.length < 6) ? null : Integer.parseInt(args[5]);
        Integer numRandomWalkers = (args.length < 7) ? null : Integer.parseInt(args[6]);
        return run(numMappers, numNodes, tmpOutput, width, wrapMultiplier, numPriorityWalkers, numRandomWalkers);
    ...
    public int runRandomInputGenerator(int numMappers, long numNodes, Path tmpOutput,
        Integer width, Integer wrapMultiplier, Integer numPriorityWalkers, Integer numRandomWalkers)
        throws Exception {
      ...
      setJobConf(job, numMappers, numNodes, width, wrapMultiplier, numPriorityWalkers, numRandomWalkers);
    ...
    public int run(int numMappers, long numNodes, Path tmpOutput,
      ...
      return runGenerator(numMappers, numNodes, tmpOutput, width, wrapMultiplier, numPriorityWalkers,
          numRandomWalkers);
    }
  ...
  static class Loop extends Configured implements Tool {
    ...
    protected void runGenerator(int numMappers, long numNodes,
        String outputDir, Integer width, Integer wrapMultiplier, Integer numPriorityWalkers, Integer numRandomWalkers)
        throws Exception {
    ...
    public int run(String[] args) throws Exception {
      ...
      try {
        ...
        Integer numPriorityWalkers = (args.length < 8) ? 0 : Integer.parseInt(args[7]);
        Integer numRandomWalkers = (args.length < 9) ? 0 : Integer.parseInt(args[8]);
        ...
        for (int i = 0; i < numIterations; i++) {
          ...
          runGenerator(numMappers, numNodes, outputDir, width, wrapMultiplier, numPriorityWalkers, numRandomWalkers);
  ...
  private static void setJobConf(Job job, int numMappers, long numNodes,
    ...
    if (numPriorityWalkers != null) {
      job.getConfiguration().setInt(CONCURRENT_WALKER_PRIORITY_KEY, numPriorityWalkers);
    }
    if (numRandomWalkers != null) {
      job.getConfiguration().setInt(CONCURRENT_WALKER_RANDOM_KEY, numRandomWalkers);
    }
  }
